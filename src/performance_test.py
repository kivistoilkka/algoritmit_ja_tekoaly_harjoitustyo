...
def run_compression_test(service: TextCompressorService, original_file_path, compressed_file_path, compression_method):
    ...
    if encoding_successful:
        ...
        print(f"Original file size {file_size_original} bytes, compressed size {file_size_compressed}")
        print(f"Compression ratio {(file_size_compressed/file_size_original)*100:.2f} %")
    else:
...
def run_decompression_test(service: TextCompressorService, original_file_path, compressed_file_path, decompressed_file_path, compression_method):
    ...
    if decoding_successful:
        ...
        print(f"File sizes equal: {file_size_decompressed == file_size_original}")
    else:
...
def main():
    ...
    test_text_files = [
        'src/tests/performance_test_files/alice_huffman_compressed'
        ,'src/tests/performance_test_files/alice_huffman_decompressed'
        ,'src/tests/performance_test_files/alice_lzw_compressed'
        ,'src/tests/performance_test_files/alice_lzw_decompressed'
        ,'src/tests/performance_test_files/frankenstein_huffman_compressed'
    ...
    run_compression_test(service, original_file_path, compressed_file_path, 'huffman coding')

    print('\n# Decompress Alice')
    decompressed_file_path = 'src/tests/performance_test_files/alice_huffman_decompressed'
    run_decompression_test(service, original_file_path, compressed_file_path, decompressed_file_path, 'huffman coding')
    ...
    run_compression_test(service, original_file_path, compressed_file_path, 'lzw')

    print('\n# Decompress Alice')
    decompressed_file_path = 'src/tests/performance_test_files/alice_lzw_decompressed'
    run_decompression_test(service, original_file_path, compressed_file_path, decompressed_file_path, 'lzw')
